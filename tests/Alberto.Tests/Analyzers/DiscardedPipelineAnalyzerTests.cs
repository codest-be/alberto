...
namespace Alberto.Tests.Analyzers;

/// <summary>
/// Runs <see cref="DiscardedPipelineAnalyzer"/> (ALB2001) against real compilations that reference
/// the real <c>Alberto.Commands</c>, so the type matching is exercised against the shipped
...
public class DiscardedPipelineAnalyzerTests
{
    ...
    private static IReadOnlyList<MetadataReference> BuildReferenceSet()
    {
        ...
        // enough: it pulls in Alberto.Commands, but the probe also names DcbQuery, IEvent and
        // Decision, which live in Alberto.
