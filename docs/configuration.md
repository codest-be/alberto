...
therefore record only the tag **concepts** (`order,customer`) by default, which is what identifies
the consistency boundary the append was checked against. Set `RecordEventTagValues` to `true` to emit
...
| Catalog database | Postgres options | none (required) | `Tenancy:Catalog:*` |
| Per-shard database | Postgres options | none | `Tenancy:Shards:{shardId}:*` |
...
> **Not covered:** pointing a module at an *empty* database, whether from a renamed schema,
> a wrong connection string or a lost volume. There is nothing to contradict, so it migrates
