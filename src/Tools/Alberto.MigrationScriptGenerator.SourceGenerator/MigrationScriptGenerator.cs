...
public class MigrationScriptGenerator : IIncrementalGenerator
{
    ...
    {
        // Find all types marked with [GenerateMigration] attribute
        var projectionTypesWithAttribute = context.SyntaxProvider
            .CreateSyntaxProvider(
                ...
                transform: static (ctx, _) => GetProjectionTypeWithAttribute(ctx))
        ...
        {
            var projectionTypes = source.Left.Where(p => p != null).ToList()!;
    ...
    private static ProjectionTypeInfo? GetProjectionTypeWithAttribute(GeneratorSyntaxContext context)
    {
        ...
        // Find the GenerateMigration attribute
        ...
        // Use type name in lowercase if table name not specified
        tableName ??= symbol.Name.ToLowerInvariant();

        return new ProjectionTypeInfo(schema!, tableName, symbol.Name);
    }
...
/// </summary>
public static class Migration_{migrationNumber:D3}_{migrationKey}
{{
    public const int Number = {migrationNumber};
    public const string Key = ""{migrationKey}"";
    public const string Sql = @""{escapedSql}"";
}}
";
    }
...
";
    ...
    private class ProjectionTypeInfo
    {
        public ProjectionTypeInfo(string schema, string tableName, string typeName)
        {
            Schema = schema;
            TableName = tableName;
            TypeName = typeName;
