...
DO $$ BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = '$schema$' AND tablename = 'tenant_leases') THEN
        ALTER TABLE $schema_prefix$tenant_leases RENAME TO alberto_tenant_leases;
    END IF;
END $$;

DO $$ BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = '$schema$' AND tablename = 'tenant_assignments') THEN
        ALTER TABLE $schema_prefix$tenant_assignments RENAME TO alberto_tenant_assignments;
    END IF;
END $$;
