-- Alberto DCB Event Store – 003 Append Set-Based (Multi-Tenant)
--
...
-- events in one batch.
--
-- The four functions are replaced with CREATE OR REPLACE FUNCTION so this
-- script is idempotent.  The RETURNS TABLE signature, parameter names, and
-- all observable behaviour (returned column order, pg_notify payload and
-- semantics, DCB conflict checks) are unchanged.
--
...
-- materialised into v_rows, not from a re-read of alberto_events.  A re-read
-- would be unsafe in the multi-tenant variant: the per-tenant advisory lock allows
-- concurrent appends from different tenants to interleave on the shared
-- global_position sequence, so a positional range query can silently omit events.
--
