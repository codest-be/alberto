-- Alberto DCB Event Store - Query Functions (Multi-Tenant)
--
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_append_events(p_tenant_id character varying, p_events jsonb, p_dcb_types character varying[] DEFAULT NULL::character varying[], p_dcb_tags character varying[] DEFAULT NULL::character varying[], p_expected_position bigint DEFAULT NULL::bigint) RETURNS TABLE(global_position bigint, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_event JSONB;
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_append_events_v3(p_tenant_id character varying, p_events jsonb, p_dcb_types character varying[] DEFAULT NULL::character varying[], p_dcb_all_tags character varying[] DEFAULT NULL::character varying[], p_expected_position bigint DEFAULT NULL::bigint) RETURNS TABLE(global_position bigint, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_event JSONB;
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_append_events_v4(p_tenant_id character varying, p_events jsonb, p_dcb_types character varying[] DEFAULT NULL::character varying[], p_dcb_tags character varying[] DEFAULT NULL::character varying[], p_expected_position bigint DEFAULT NULL::bigint) RETURNS TABLE(global_position bigint, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_event JSONB;
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_append_events_v6(p_tenant_id character varying, p_events jsonb, p_dcb_types character varying[] DEFAULT NULL::character varying[], p_dcb_all_tags character varying[] DEFAULT NULL::character varying[], p_expected_position bigint DEFAULT NULL::bigint) RETURNS TABLE(global_position bigint, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_event JSONB;
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_get_last_position(p_tenant_id character varying) RETURNS bigint
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_position BIGINT;
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_get_last_position_global() RETURNS bigint
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_position BIGINT;
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_maintain_tenants_batch() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_pick_all_tags_driver(p_tenant_id character varying, p_tags character varying[], p_after_position bigint, p_limit integer) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_all(p_tenant_id character varying, p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_all_global(p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_all_tags(p_tenant_id character varying, p_tags character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_driver VARCHAR(500);
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_tags(p_tenant_id character varying, p_tags character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
BEGIN
    IF p_tags IS NULL OR array_length(p_tags, 1) IS NULL THEN
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_types(p_tenant_id character varying, p_types character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_types_and_all_tags(p_tenant_id character varying, p_types character varying[], p_tags character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_driver VARCHAR(500);
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_types_and_tags(p_tenant_id character varying, p_types character varying[], p_tags character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_tag  VARCHAR(500);
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_types_or_all_tags(p_tenant_id character varying, p_types character varying[] DEFAULT NULL::character varying[], p_tags character varying[] DEFAULT NULL::character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_driver VARCHAR(500);
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_read_by_types_or_tags(p_tenant_id character varying, p_types character varying[] DEFAULT NULL::character varying[], p_tags character varying[] DEFAULT NULL::character varying[], p_after_position bigint DEFAULT 0, p_limit integer DEFAULT NULL::integer) RETURNS TABLE(global_position bigint, tenant_id character varying, event_id uuid, event_type character varying, event_tags character varying[], event_data jsonb, event_metadata jsonb, created_at timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION $schema_prefix$alberto_save_checkpoint_if_lease_held(p_processor_id text, p_consumer_id text, p_replica_id text, p_position bigint) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_written INTEGER;
