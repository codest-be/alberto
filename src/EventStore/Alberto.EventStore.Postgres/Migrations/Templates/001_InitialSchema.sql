...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1
            ...
            CREATE TABLE {schema}.__alberto_schema_version
            (
                migration_name
                VARCHAR
            (
    ...
    WHERE migration_name = '001_InitialSchema') THEN
            ...
            CREATE INDEX IF NOT EXISTS idx_{schema} _subscription_checkpoints_updated
            ...
            CREATE INDEX IF NOT EXISTS idx_{schema} _poison_pills_subscription
                ON {schema}.subscription_poison_pills (subscription_id);

            CREATE INDEX IF NOT EXISTS idx_{schema} _poison_pills_event
                ON {schema}.subscription_poison_pills (event_id);

            CREATE INDEX IF NOT EXISTS idx_{schema} _poison_pills_unresolved
                ON {schema}.subscription_poison_pills (subscription_id, last_failed_at)
                WHERE resolved_at IS
            NULL;

            -- Record migration
            INSERT INTO {schema}.__alberto_schema_version (migration_name, applied_at, is_breaking, description)
