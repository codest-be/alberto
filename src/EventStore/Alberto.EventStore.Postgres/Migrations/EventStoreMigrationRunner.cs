...
/// </summary>
public sealed class EventStoreMigrationRunner(
    string connectionString,
    ILogger logger) : IEventStoreMigrationRunner
{
    ...
    public async Task<bool> MigrateAsync(string schema, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Running EventStore migrations for schema: {Schema}", schema);
        ...
        try
        {
            ...
            _logger.LogInformation("EventStore migrations completed successfully for schema: {Schema}", schema);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception during EventStore migration for schema: {Schema}", schema);
            return false;
        }
    }
}
...
/// <summary>
/// Interface for running database migrations for the event store.
/// Implement this interface to use custom migration tools.
/// </summary>
public interface IEventStoreMigrationRunner
{
    /// <summary>
