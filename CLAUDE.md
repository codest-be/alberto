...
- **A handler failure that coincides with shutdown is reported as a clean stop.** In the pipelined path `ControlLoop.IsFaulted` is only set when `pipelineFailure` was recorded; the outer `catch (OperationCanceledException) when (ct.IsCancellationRequested)` swallows the failure when cancellation wins the race. `DiscoveredIssuesTests.Pipelined_NonShutdownOce_FaultsPipeline_AndCheckpointStaysBehindFaultingEvent` fails on roughly half of all runs because of this — measured 5 failures in 10 runs on a clean checkout at `c5e973e`, so it is pre-existing and not caused by any current change. The test's own comment claiming "this is not a wall-clock race" is wrong: its `allDispatched` gate fires on handler *entry* (the `Interlocked.Increment` precedes the `throw`), so the test cancels before the exception reaches the pipeline. Only the fault *signal* is unreliable — the checkpoint assertion passes consistently, so at-least-once delivery is unaffected.
- **`getRecentPayments` is not tenant-scoped.** `PaymentSummaryProjection` keys one document per `PaymentId`, and `PaymentsModule` declares no tenancy at all — `PaymentSummary` carries no tenant column, so there is nothing to filter on. The resolver therefore returns individual payment records from every tenant regardless of `X-Tenant-Id`. The per-request `tenantId` it used to pass was decoration that only ever made the query return nothing (see the closed gap below); removing it exposed the underlying absence of tenancy rather than creating it. The two `*Overview` projections are genuinely cross-tenant aggregates — a single document accumulating all events — so reading them without a tenant is correct. Tenant-scoping `getRecentPayments` needs a tenant on the payments projection first; `OrderSummaryEfProjection` shows the shape, persisting the tenant as a column that queries filter on.
