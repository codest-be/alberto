...
/// <summary>
/// Tests for multi-schema support in PostgreSQL EventStore
...
public class MultiSchemaEventStoreTests(PostgresTestFixture fixture) : IAsyncLifetime
{
    ...
    private async Task<string> LoadMigrationFromFile()
    {
        ...
        while (solutionDirectory != null && !Directory.Exists(Path.Combine(solutionDirectory.FullName, "EventStore.Postgres")))
            solutionDirectory = solutionDirectory.Parent;

        if (solutionDirectory == null)
            throw new DirectoryNotFoundException("Could not locate EventStore.Postgres directory");

        var migrationPath = Path.Combine(solutionDirectory.FullName, "EventStore.Postgres", "Migrations", "CreateEventStoreSchema.sql");
